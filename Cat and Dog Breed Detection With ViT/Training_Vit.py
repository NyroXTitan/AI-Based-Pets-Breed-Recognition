...
from visualize_and_equalize_class_distribution import visualize_class_distribution, equalize_class_distribution
import json
import os

# ✅ Data Loader
def load_data(data_dir, processor, is_train=False, target_count=200):
    ...
    if is_train:
        print("📊 Class distribution before equalizing class distribution...")
        visualize_class_distribution(raw_dataset, "Before equalizing class distribution")
        raw_dataset = equalize_class_distribution(raw_dataset, target_count=target_count)
        print("📈 Class distribution after equalizing class distribution...")
        visualize_class_distribution(raw_dataset, "After equalizing class distribution", class_names=ImageFolder(data_dir).classes)
...
def main():
    ...
    train_dataset = CustomDataset(load_data(train_dir, processor, is_train=True, target_count=200))
